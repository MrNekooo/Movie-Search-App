        ...
        <div>
            ...
            <ul className='grid grid-cols-5 gap-5 min-h-150 max-2xl:grid-cols-4 max-xl:grid-cols-3 max-lg:grid-cols-2 max-sm:grid-cols-1'>
                {favorites.map(favorite => (
                    <li key={favorite.id} className={`flex flex-col justify-between items-center bg-black/50 w-full cursor-pointer hover:scale-101 transition-all duration-300 gap-3 px-3 py-5 rounded-2xl max-sm:w-70 m-auto ${deleteIds.has(favorite.id) && "opacity-0"} `}
                        onClick={() => navigate(`/movie/${favorite.id}`)}>
